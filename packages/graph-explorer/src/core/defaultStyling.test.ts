...
describe("resolveDefaultStyling", () => {
  it("should convert lucide icon name shorthand to a lucide:<name> reference", () => {
    const data = {
      vertices: {
    ...
    expect(result.vertices?.[0].iconUrl).toBe("lucide:user");
    expect(result.vertices?.[0].iconImageType).toBe("image/svg+xml");
    expect(result.vertices?.[0].color).toBe("#1565C0");
  });

  it("should prefer explicit iconUrl over icon shorthand", () => {
    const data = {
      vertices: {
        User: {
          icon: "user",
  ...
  it("should pass through unknown lucide names — the render-time resolver handles missing icons", () => {
    const data = {
      vertices: {
    ...
    // Unknown names still produce a lucide ref; render-time decides to draw
    // nothing rather than failing the import.
    expect(result.vertices?.[0].iconUrl).toBe("lucide:not-a-real-icon");
    expect(result.vertices?.[0].color).toBe("#1565C0");
  });

  it("should accept full lucide:<name> iconUrl values", () => {
    const data = {
      vertices: {
        User: {
    ...
    expect(result.vertices?.[0].iconUrl).toBe("lucide:plane");
    expect(result.vertices?.[0].iconImageType).toBe("image/svg+xml");
  });

  it("should resolve edge styles", () => {
    const data = {
      edges: {
  ...
  it("should handle empty data", () => {
    const result = resolveDefaultStyling({});
    expect(result.vertices).toHaveLength(0);
    expect(result.edges).toHaveLength(0);
  });

  it("should handle multiple vertex types", () => {
    const data = {
      vertices: {
        User: { color: "#1565C0" },
  ...
  it("should resolve all vertex style properties", () => {
    const data = {
      vertices: {
        User: {
          color: "#1565C0",
  ...
  it("should resolve iconUrl and iconImageType when explicitly provided", () => {
    const data = {
      vertices: {
        User: {
  ...
  it("should preserve zero values for backgroundOpacity and borderWidth", () => {
    const data = {
      vertices: {
        User: { backgroundOpacity: 0, borderWidth: 0 },
      },
