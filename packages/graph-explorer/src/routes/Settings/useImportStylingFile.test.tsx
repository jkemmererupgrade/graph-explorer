...
describe("useImportStylingFile", () => {
  ...
  it("should convert lucide icon shorthand to lucide:<name> references on import", async () => {
    const state = new DbState();
    state.activeStyling = {};
    const { result } = renderHookWithState(() => useImportStylingFile(), state);
    ...
    expect(imported.vertices?.find(v => v.type === "User")?.iconUrl).toBe(
      "lucide:user",
    );
    expect(imported.vertices?.find(v => v.type === "User")?.iconImageType).toBe(
      "image/svg+xml",
    );
  });
